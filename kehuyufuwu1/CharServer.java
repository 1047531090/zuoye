...
public class CharServer extends JFrame {
 ...
 public static synchronized void disconnect(ClientProcessor client, boolean toRemoveFromList){
  try {
   ...
   CharServer.activeConnects--; //将连接数减1
   String constr = "目前有" + CharServer.activeConnects + "客户相连";
 ...
 public static void main(String[] args) {
  CharServer chatServer1 = new CharServer();
  ...
  while (true) {
   ...
   if (clientProcessors.size() < Constants.MAX_CLIENT) {
    ...
    if (checkClient(c)) {
     ...
     // 如果客户端合法，则继续
     int connum = ++CharServer.activeConnects;
     // 在状态栏里显示连接数
     String constr = "目前有" + connum + "客户相连";
     CharServer.statusBar.setText(constr);
     // 将客户socket信息写入list框
     CharServer.connectInfoList.add(c.clientIP + "连接"); 
     c.start();
     // 通知所有客户端用户列表发生变化
     notifyRoomPeople();
    } else {
	 ...
	 public void run() {
	  while (running) {
	   ...
	   try {
	    ...
	    System.out.println("Error" + e);
	    CharServer.disconnect(this);
	    CharServer.notifyRoomPeople();
	    return;
	   }
	    //客户已离开
	   if (line == null){
	   ...
	   if (keyword.equals(Constants.MSG_IDENTIFER)){
	    ...
	    CharServer.sendMsgToClients(msg); 
